...
AjaxControlToolkit.MaskedEditBehavior = function(element) 
{
   ...
   AjaxControlToolkit.MaskedEditBehavior.prototype = { 
    ...
    , _PeforformValidLostFocus : function(isblur) 
    {
        ...
        //Added by Siva Kumar Thangaraj
        //Included to avoid unneccessary commas after lostfocus
        ValueText = wrapper.get_Value();

            if(!this._AutoComplete && ValueText.indexOf('.')>=0)
            {
                //Included to avoid 10 instead of 1 and 1.0 instead of 0.61
                var dotIndex = this._LogicMask.indexOf('.');
                var loop = dotIndex+1;
                if (dotIndex > 0)
                {
                    var valIndex = ValueText.indexOf('.') + 1;
                    for(;loop<this._LogicMask.length; loop++)
                    {
                        if(ValueText.charAt(valIndex)=='')
        ...
        if (this._ClearMaskOnLostfocus)
        {
            this._InitValue(ValueText,false);
            ValueText = wrapper.get_Value();
            ValueText = ValueText.replace('_','0');
            wrapper.set_Value(this._getClearMask(wrapper.get_Value));
        }
        else
        {
            this._InitValue(ValueText.substring(this._LogicFirstPos,this._LogicLastPos+1),false);
        }
        
        //End of Added by Siva Kumar Thangaraj
        return IsValid;
    }
...
//                    }
                    ...
                    //Modified by Siva Kumar Thangaraj
                    if ((this._isValidMaskedEditPosition(curpos+1+i)|| resttext.charAt(i) == '.') && restlogi.substring(i,i+1) != this._LogicPrompt)
                    {
                        if(resttext.charAt(i) == '.')
                            masktext = masktext.substring(0,posaux- this._LogicFirstPos-qtdDt) + '_' + masktext.substring(posaux+1- this._LogicFirstPos-qtdDt);
                        else
                            masktext = masktext.substring(0,posaux- this._LogicFirstPos-qtdDt) + resttext.substring(i,i+1) + masktext.substring(posaux+1- this._LogicFirstPos-qtdDt);
                        logiTxt = logiTxt.substring(0,posaux- this._LogicFirstPos-qtdDt) + restlogi.substring(i,i+1) + logiTxt.substring(posaux+1- this._LogicFirstPos-qtdDt);
                        posaux = this._getNextPosition(posaux+1);
    ...
    , AutoFormatNumber : function()
    {
        ...
        for (i = this._LogicFirstPos ; i <= this._LogicLastPos ; i++) 
        {
            if (this._LogicTextMask.substring(i,i+1) == this._LogicPrompt )
            {
                ...
                {
                    this._LogicTextMask = 
                    this._LogicTextMask.substring(0,i) + CharComp + this._LogicTextMask.substring(i+1);
