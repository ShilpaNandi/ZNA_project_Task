...
update armis_los_exc with(rowlock)
set 
exc_paid_idnmty_amt			=	round(isnull(exc_paid_idnmty_amt,0),0),
exc_paid_exps_amt			=	round(isnull(exc_paid_exps_amt,0),0),
exc_resrvd_idnmty_amt		=	round(isnull(exc_resrvd_idnmty_amt,0),0),
exc_resrvd_exps_amt			=	round(isnull(exc_resrvd_exps_amt,0),0),
non_bilabl_paid_idnmty_amt	=	round(isnull(non_bilabl_paid_idnmty_amt,0),0),
non_bilabl_paid_exps_amt	=	round(isnull(non_bilabl_paid_exps_amt,0),0),
non_bilabl_resrvd_idnmty_amt=	round(isnull(non_bilabl_resrvd_idnmty_amt,0),0),
non_bilabl_resrvd_exps_amt	=	round(isnull(non_bilabl_resrvd_exps_amt,0),0),
exc_ldf_ibnr_amt			=	round(isnull(exc_ldf_ibnr_amt,0),0),
subj_ldf_ibnr_amt			=	round(isnull(subj_ldf_ibnr_amt,0),0)
where custmr_id = @custmr_id
and prem_adj_pgm_id = @prem_adj_pgm_id
and actv_ind = 1
