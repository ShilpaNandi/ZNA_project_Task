...
{
    private Prem_Adj_Dpst_ChbxBS PremAdjDpstChbxInfo = new Prem_Adj_Dpst_ChbxBS();
    ...
    private void CheckExitWithoutSave()
    {
        ...
        list.Add(rbtnCHFUseDpst);
        list.Add(rbtnCHFUseDpst2);
    ...
    /// <summary>
    /// bool Function to check the fist row of CHF Use deposit record is new or existing. Used while saving the record
    /// </summary>
    /// <param name=""></param>
    /// <returns>bool(True/False)</returns>
    ...
    /// <summary>
    /// bool Function to check the second row of CHF Use deposit record is new or existing. Used while saving the record
    /// </summary>
    /// <param name=""></param>
    /// <returns>bool(True/False)</returns>
    ...
    private IList<Prem_Adj_Dpst_ChbxBE> BindlistChbx
    {
        get
        {
            if (RetrieveObjectFromSessionUsingWindowName("BindlistChbx") == null)
                SaveObjectToSessionUsingWindowName("BindlistChbx", new List<Prem_Adj_Dpst_ChbxBE>());
    ...
    private void BindLBADetails(int prgprdID)
    {
        try
        {
            ...
            if (PlcyLst.Count > 0)
            {
                ...
                int AdjParmSetupID = LBAParamBS.getAdjParamtr(prgprdID, CustomerID).FirstOrDefault().adj_paramet_setup_id;
                ...
                if (BindlistChbx != null)
                {
                    foreach (Prem_Adj_Dpst_ChbxBE PremAdjDpstChbxBE in BindlistChbx)
                    {
                        ...
                        {
                            AdjustmentParameterSetupBE LBAParamLBABE = new AdjustmentParameterSetupBE();
                            if (LBAParamLBABE.incld_ernd_retro_prem_ind == true)
                            { //Upade Button
                                ...
                                ViewState["ADJCHFDpstChbxID"] = premAdjDpstChbxBE.prem_adj_dpst_chbx_id;
    ...
    protected Prem_Adj_Dpst_ChbxBE SaveDpstChbxCHF(Prem_Adj_Dpst_ChbxBE PremAdjDpstChbxCHFBE, AdjustmentParameterSetupBE AdjStupCHFBE)
    {
        ...
        if (ResultCHF)
        {
            ...
            //    {
            //        Prem_Adj_Dpst_ChbxBE PremAdjDpstChbxBE = new Prem_Adj_Dpst_ChbxBE();
            //        PremAdjDpstChbxBE = PremAdjDpstChbxInfo.getAdjParamRow(int.Parse(ViewState["ADJCHFDpstChbxID2"].ToString()));
            //    }
            //    bool aptflg = AdjParameterTransactionWrapper.SubmitTransactionChanges();
            //    ShowConcurrentConflict(aptflg, AdjParameterTransactionWrapper.ErrorMessage);
            //}
    ...
    protected Prem_Adj_Dpst_ChbxBE SaveDpstChbxCHF2(Prem_Adj_Dpst_ChbxBE PremAdjDpstChbxCHFBE, AdjustmentParameterSetupBE AdjStupCHFBE)
    {
        ...
        if (ResultCHF)
        {
            ...
            {
                Prem_Adj_Dpst_ChbxBE PremAdjDpstChbxBE = new Prem_Adj_Dpst_ChbxBE();
                PremAdjDpstChbxBE = PremAdjDpstChbxInfo.getAdjParamRow(int.Parse(ViewState["ADJCHFDpstChbxID"].ToString()));
            }
            bool aptflg = AdjParameterTransactionWrapper.SubmitTransactionChanges();
            ShowConcurrentConflict(aptflg, AdjParameterTransactionWrapper.ErrorMessage);
        }
        ...
        try
        {
            ...
            if (PolCountCHF > 0)
            {
                ...
                if (!Flag)
                {
                    ...
                    //    string AdjReviewResultCHF = PremAdjDpstChbxBS.getAdjParamResultOther(prem_adj_pgm_setup_id, int.Parse(ViewState["PRGPRDID"].ToString()), AISMasterEntities.AccountNumber, Lbablaccess.GetLookUpID("CHF", "ADJUSTMENT PARAMETER TYPE"));
    ...
    protected void DisableCHFfirstRow(object sender, CommandEventArgs e)
    {
        try
        {
            ...
            //    //if (ViewState["ADJCHFPRGID"] != null)
            //{
            //    int premAdjDpstChbxID = int.Parse(ViewState["ADJCHFDpstChbxID"].ToString());
            //    premAdjDpstChbxBE = PremAdjDpstChbxBS.getAdjParamRow(premAdjDpstChbxID);
            //    //int AdjParmtSetupID = int.Parse(ViewState["ADJCHFPRGID"].ToString());
            //    //adjParmStupBE = AdjPrmStupBS.getAdjParamRow(AdjParmtSetupID);
            //    //Concurrency Issue
            //    //Prem_Adj_Dpst_ChbxBE premAdjDpstChbxBEold = (BindlistChbx.Where(o => o.prem_adj_dpst_chbx_id.Equals(premAdjDpstChbxID))).First();
            //    //AdjustmentParameterSetupBE adjParmStupBEold = (Bindlist.Where(o => o.adj_paramet_setup_id.Equals(AdjParmtSetupID))).First();
            //    //bool con = ShowConcurrentConflict(Convert.ToDateTime(adjParmStupBEold.UPDATE_DATE), Convert.ToDateTime(adjParmStupBE.UPDATE_DATE));
            //    //bool con = ShowConcurrentConflict(Convert.ToDateTime(premAdjDpstChbxBEold.UPDATE_DATE), Convert.ToDateTime(premAdjDpstChbxBE.UPDATE_DATE));
            //    if (!con)
