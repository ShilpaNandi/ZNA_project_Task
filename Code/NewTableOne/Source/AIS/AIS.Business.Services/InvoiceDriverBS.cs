...
namespace ZurichNA.AIS.Business.Logic
{
    public class InvoiceDriverBS
    {
        ...
        {
            return (new InvoiceDriverDA()).ProgramSummary_Cancel_ZDW_Info(adjNO);
        }
        //Phase-3
        public DataTable GetProgramSummary_Info_ByZdwKey(string ZDWKeytxt)
        {
            return (new InvoiceDriverDA()).GetProgramSummary_Info_ByZdwKey(ZDWKeytxt);
        }
        //Phase-3
        public DataTable GetProgramSummary_ZDW_Info(int adjNO)
        {
            return (new InvoiceDriverDA()).GetProgramSummary_ZDW_Info(adjNO);
        }
        //Phase-3
        public DataTable UpdateDraftZDWKey_ProgramSummary(int adjNO, string strZDWKey, int intUserID)
        {
            return (new InvoiceDriverDA()).UpdateDraftZDWKey_ProgramSummary(adjNO, strZDWKey, intUserID);
        }
        //Phase-3
        public DataTable UpdateFinalZDWKey_ProgramSummary(int adjNO, string strZDWKey, int intUserID)
        {
            return (new InvoiceDriverDA()).UpdateFinalZDWKey_ProgramSummary(adjNO, strZDWKey, intUserID);
        }
    }
}
