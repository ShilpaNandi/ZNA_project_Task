...
namespace ZurichNA.AIS.DAL.Logic
{
    public class BusinessUnitOfficeDA : DataAccessor<INT_ORG, BusinessUnitOfficeBE, AISDatabaseLINQDataContext>
    {
        ...
        public IList<LookupBE> GetBUForLookups()
        {
            IList<LookupBE> BusinessUnits = new List<LookupBE>();

            IQueryable<LookupBE> result = from res in this.Context.INT_ORGs
                                          where (res.bsn_unt_cd != "N")
                                          orderby (res.bsn_unt_cd)
                                          where res.actv_ind == true
                                          select new LookupBE
                                          {
                                              LookUpName = res.bsn_unt_cd,
                                              LookUpID = res.int_org_id
                                          };

            BusinessUnits = result.ToList().Distinct(new DistinctBU()).ToList();

            return BusinessUnits;
        }

        public class DistinctBU : IEqualityComparer<LookupBE>
        {
            public bool Equals(LookupBE x, LookupBE y)
            {
                return x.LookUpName.Equals(y.LookUpName);
            }

            public int GetHashCode(LookupBE obj)
            {
                return obj.LookUpName.GetHashCode();
            }
        }

        public IList<LookupBE> GetOfficeForLookups()
        {
            IList<LookupBE> BusinessUnits = new List<LookupBE>();

            IQueryable<LookupBE> result = from res in this.Context.INT_ORGs
                                          where res.city_nm != null && res.city_nm != "A" && res.city_nm != ""
                                          orderby res.city_nm == null ? "" : res.city_nm
                                          where res.actv_ind == true
                                          select new LookupBE
                                          {
                                              LookUpID = res.int_org_id,
                                              LookUpName = (res.city_nm == null ? "" : res.city_nm)
                                          };
            BusinessUnits = result.ToList().Distinct(new DistinctBU()).ToList();
