if exists (select 1 from sysobjects where name = 'GetTexasTax' and type = 'P')
           drop procedure GetTexasTax
go
SET ANSI_NULLS OFF
GO
...
CREATE PROCEDURE [dbo].[GetTexasTax]
@ADJNO INT,
@FLAG INT,
@HISTFLAG BIT,
@CMTCATGID INT
...
AND TAXCMPNT.LKUP_TXT LIKE Case when (LOB.LKUP_TXT='WC' and substring(TAXCMPNT.LKUP_TXT,1,4)='CHF')  then '%Claimant%' 
								when (LOB.LKUP_TXT<>'WC' and substring(TAXCMPNT.LKUP_TXT,1,4)='CHF') then '%Loss%' 
								when substring(TAXCMPNT.LKUP_TXT,1,4)='LCF'                          then '%Premium%' end
...
ST.ATTR_1_TXT ASC,LOB.LKUP_TXT ASC,"TAX RATE" ASC,"INDICATOR" Desc
...
GO
if object_id('GetTexasTax') is not null
        print 'Created Procedure GetTexasTax'
else
        print 'Failed Creating Procedure GetTexasTax'
go

if object_id('GetTexasTax') is not null
        grant exec on GetTexasTax to  public
go
