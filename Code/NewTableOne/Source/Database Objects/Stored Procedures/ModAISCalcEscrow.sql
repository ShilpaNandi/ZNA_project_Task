...
-----               13/08/2010
-----				 rounding off the @adj_tot_paid_los_bil_amt before calc
-----				01/26/2012
-----				Fixed the TFS Bug: 15892 Escrow should be calculated correctly when the policy number from the misc invoicing screen is not an escrow policy
---------------------------------------------------------------------
...
--			end
		...
		--Escrow should be calculated correctly when the policy number from the misc invoicing screen is not an escrow policy 
		--(As per TFS Bug 15892 we added the adj_typ_id =62)
		select 
		...
		inner join PREM_ADJ_PERD pap on (m.prem_adj_perd_id=pap.prem_adj_perd_id)
		inner join dbo.COML_AGMT ca on (rtrim(ltrim(m.pol_sym_txt))=rtrim(ltrim(ca.pol_sym_txt)) and rtrim(ltrim(m.pol_nbr_txt))=rtrim(ltrim(ca.pol_nbr_txt)) and rtrim(ltrim(m.pol_modulus_txt))=rtrim(ltrim(ca.pol_modulus_txt)) and pap.prem_adj_pgm_id=ca.prem_adj_pgm_id)
		...
		**********************************************/
		...
		select @escrow_prev_bil_amt = CASE WHEN isnull(stp.escr_adj_amt,0)>0 THEN isnull(stp.escr_adj_amt,0) ELSE 0 END ,
		...
		*/
		...
		set @esc_amt_billed = CASE WHEN @months_held_amt>0 THEN @months_held_amt ELSE 0 END + (@previous_credits * -1)
