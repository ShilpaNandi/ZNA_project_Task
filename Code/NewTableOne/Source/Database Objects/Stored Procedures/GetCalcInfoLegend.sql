...
---- WC ADJ PRIOR
		...
		INNER JOIN COML_AGMT ON COML_AGMT.PREM_ADJ_PGM_ID = PREM_ADJ_PGM.PREM_ADJ_PGM_ID
		AND COML_AGMT.CUSTMR_ID = PREM_ADJ_PGM.CUSTMR_ID AND COML_AGMT.ACTV_IND = 1	
		INNER JOIN PREM_ADJ_STS ON PREM_ADJ_STS.PREM_ADJ_ID IN (SELECT  prior_prem_adj_id FROM PREM_ADJ_PGM PGM
							INNER JOIN PREM_ADJ_PERD  ON 
							PREM_ADJ_PERD.PREM_ADJ_ID = @ADJNO
							AND PGM.PREM_ADJ_PGM_ID = PREM_ADJ_PERD.PREM_ADJ_PGM_ID
							AND PGM.CUSTMR_ID = PREM_ADJ_PERD.CUSTMR_ID 
							AND PGM.ACTV_IND = 1 
							AND PGM.PREM_ADJ_PGM_ID=PREM_ADJ_PGM.PREM_ADJ_PGM_ID  ) 
		AND PREM_ADJ_STS.PREM_ADJ_STS_ID IN (SELECT MAX(PREM_ADJ_STS.PREM_ADJ_STS_ID) from PREM_ADJ_STS WHERE
		PREM_ADJ_STS.ADJ_STS_TYP_ID in (349,352) AND PREM_ADJ_STS.EXPI_DT IS NULL AND PREM_ADJ_STS.PREM_ADJ_ID IN (SELECT  prior_prem_adj_id FROM PREM_ADJ_PGM PGM
							INNER JOIN PREM_ADJ_PERD  ON 
							PREM_ADJ_PERD.PREM_ADJ_ID = @ADJNO
							AND PGM.PREM_ADJ_PGM_ID = PREM_ADJ_PERD.PREM_ADJ_PGM_ID
							AND PGM.CUSTMR_ID = PREM_ADJ_PERD.CUSTMR_ID 
		...
		WHERE PREM_ADJ_PERD.PREM_ADJ_ID 
						IN (SELECT  prior_prem_adj_id FROM PREM_ADJ_PGM PGM
							INNER JOIN PREM_ADJ_PERD  ON 
							PREM_ADJ_PERD.PREM_ADJ_ID = @ADJNO
							AND PGM.PREM_ADJ_PGM_ID = PREM_ADJ_PERD.PREM_ADJ_PGM_ID
							AND PGM.CUSTMR_ID = PREM_ADJ_PERD.CUSTMR_ID 
...
SELECT	DISTINCT	isnull(#t_CalcInfoWCCurrent.PREM_ADJ_PGM_ID,#FinalCurrent.PREM_ADJ_PGM_ID) AS PREM_ADJ_PGM_ID,
			isnull(#t_CalcInfoWCCurrent.PERD_ID,#FinalCurrent.PERD_ID) AS PERD_ID,
			isnull(#t_CalcInfoWCCurrent.PREM_ADJ_ID,@ADJNO) As PREM_ADJ_ID,
