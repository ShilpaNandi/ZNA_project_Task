if exists (select 1 from sysobjects where name = 'ModAISCalcILRF' and type = 'P')
...
-----	Modified:	06/30/2010	Venkat Kolimi
-----			- As pe the TFS issue 12011, LDF Amount for LA WC is Repeated into All LDF fields - has LCF Aggregate
-----             Now we modified the code to get the correct LCF amount based on the Policy and LOB.
-----	Modified:	06/14/2010	Venkat Kolimi
-----			- As SR 325928 , we have added the logic to add the other amount for the adjusted prior year amount
-----			  fro the reserves,so that users have the option to adjust the amount as required.
-----			07/16/2015- Venkat Kolimi
-----			Fixed the issue with INC0877457(PLB Cumulative Loss feed ). When there are two trailing Zero's we are truncating now
-----			by adding LTRIM(RTRIM(SUBSTRING(ca.pol_nbr_txt, PATINDEX('%[^0 ]%', ca.pol_nbr_txt + ' '), LEN(ca.pol_nbr_txt) )))
...
declare	@use_paid_los_ind bit,
		...
		@state_counter int,
		--SR 325928
		@ilrf_other_amt decimal(15,2),
		--Rounding Issue
		@tot_amt_rounded decimal(15,2),
		@curr_amt_rounded decimal(15,2),
		@lim_amt_rounded decimal(15,2),
		@diff_rounded decimal(15,2),
		@aggr_chf_amt decimal(15,2),
		@prior_chf_amt decimal(15,2),
		@prior_ILRF_chf_amt decimal(15,2),
...
--					print' @fnl_overrid_amt: ' + convert(varchar(20), @fnl_overrid_amt )  
					...
					***********************************************************/
					...
					begin
						...
						and st_id = @state_id
						and actv_ind = 1 
						and valn_dt	= @prem_adj_valn_dt --added as part of the Merit Resources LDF issue
						and ((prem_adj_id is null) or (prem_adj_id = @premium_adjustment_id)) 
					end
...
--						print 'LDF LCF evaluated @use_resv_alae_ind: ' + convert(varchar(20), @use_resv_alae_ind ) 
		...
		--Bug Fix 12011
		select @ibnr_ldf_ft_id=incur_but_not_rptd_los_dev_fctr_id 
		...
		while @@Fetch_Status = 0
			begin
				begin
					...
					set @lba_result = null
					set @ldf_result = null
					set @ibnr_result=null
					set @ldf_ibnr_amt=null
...
--------end: changes for bug # 10235------------
		...
		select distinct coml_agmt_id 
		into #ILRF_Policy
		from 
		dbo.PREM_ADJ_PARMET_SETUP psl
		inner join dbo.PREM_ADJ_PARMET_DTL pdl on psl.prem_adj_parmet_setup_id=pdl.prem_adj_parmet_setup_id
		where psl.prem_adj_id=@premium_adjustment_id 
		and psl.adj_parmet_typ_id=400
		and psl.prem_adj_perd_id=@premium_adj_period_id
		...
		**********************/
		...
		begin
		   ...
		   ,[recv_typ_id]
			...
			and post_trns_typ_id in (31,29,30,34,32,33,70,71,140)
			and actv_ind = 1
		...
		select 
		@aggr_chf_amt = sum( isnull(pdc.tot_amt,0) )  --as CHF
		from 
		dbo.PREM_ADJ_PARMET_SETUP psc 
		inner join dbo.PREM_ADJ_PARMET_DTL pdc on psc.prem_adj_parmet_setup_id=pdc.prem_adj_parmet_setup_id
		inner join #ILRF_Policy lp on lp.coml_agmt_id=pdc.coml_agmt_id
		where psc.prem_adj_id=@premium_adjustment_id and psc.adj_parmet_typ_id=398

		select 
		@prior_ILRF_chf_amt=sum( isnull(pdc.tot_amt,0) )  --as Prior CHF
		from 
		dbo.PREM_ADJ_PARMET_SETUP psc 
		inner join dbo.PREM_ADJ_PARMET_DTL pdc on psc.prem_adj_parmet_setup_id=pdc.prem_adj_parmet_setup_id
		inner join #ILRF_Policy lp on lp.coml_agmt_id=pdc.coml_agmt_id
		where psc.prem_adj_id=@prev_valid_adj_id 
		and psc.adj_parmet_typ_id=398
		and psc.prem_adj_pgm_id=@premium_adj_prog_id

		--drop table #ILRF_Policy
		...
		--	set curr_amt = round(isnull(@aggr_auto_tpd_amt,0),0)
		update [dbo].[PREM_ADJ_LOS_REIM_FUND_POST]
		set curr_amt = round(isnull(@aggr_auto_tpd_amt,0),0)
		where [prem_adj_perd_id] = @premium_adj_period_id
		and [prem_adj_id] = @premium_adjustment_id
		...
		--set curr_amt = round(isnull(@aggr_gl_tpd_amt,0),0)
		update [dbo].[PREM_ADJ_LOS_REIM_FUND_POST]
		set curr_amt = round(isnull(@aggr_gl_tpd_amt,0),0)
		where [prem_adj_perd_id] = @premium_adj_period_id
		and [prem_adj_id] = @premium_adjustment_id
        ...
        --set curr_amt = round(isnull(@aggr_resv_amt,0),0)
		update [dbo].[PREM_ADJ_LOS_REIM_FUND_POST]
		set curr_amt = round(isnull(@aggr_resv_amt,0),0)
		where [prem_adj_perd_id] = @premium_adj_period_id
		and [prem_adj_id] = @premium_adjustment_id
		...
		update [dbo].[PREM_ADJ_LOS_REIM_FUND_POST]
		set curr_amt = isnull(@aggr_chf_amt,0),
		prior_yy_amt= isnull(@prior_ILRF_chf_amt,0)
		where [prem_adj_perd_id] = @premium_adj_period_id
		and [prem_adj_id] = @premium_adjustment_id
		and [custmr_id] = @customer_id
		and recv_typ_id = 140 -- lookup type: RECEIVABLE TYPE, lookup value: CHF
...
--		and pa.custmr_id = @customer_id
		...
		begin --else to: if (@ilrf_invc_lsi_ind = 0) / 'Invoice in LSI' indicator is set
			...
			begin
				...
				@prior_chf_amt = sum(ths.PerClaimFees)
				...
				inner join dbo.COML_AGMT ca on (pa.reg_custmr_id = ca.custmr_id) and (pgm.prem_adj_pgm_id = ca.prem_adj_pgm_id) 
				and (
				--ca.pol_nbr_txt = substring(pol.PolicyNumber,1,7)
					...
					then
						LTRIM(RTRIM(substring(pol.PolicyNumber,2,7))) 
					else
						LTRIM(RTRIM(pol.PolicyNumber)) 
					end
			...
			update [dbo].[PREM_ADJ_LOS_REIM_FUND_POST]
			set prior_yy_amt = isnull(prior_yy_amt,0) + isnull(@prior_chf_amt,0),
				adj_prior_yy_amt = isnull(adj_prior_yy_amt,0) + isnull(@prior_chf_amt,0)
			where [prem_adj_perd_id] = @premium_adj_period_id
			and [prem_adj_id] = @premium_adjustment_id
			and [custmr_id] = @customer_id
			and recv_typ_id = 140 -- lookup type: RECEIVABLE TYPE, lookup value: CHF
			...
			and [prem_adj_id] = @premium_adjustment_id
			and [custmr_id] = @customer_id
			and recv_typ_id in (31,29,30,34,32,33,70,140)
			...
			and [prem_adj_id] = @premium_adjustment_id
			and [custmr_id] = @customer_id
			and recv_typ_id in (31,29,30,34,32,33,70,71,140)
		...
		/********************************************************************************************
										SR 325928 Logic Starts here.
		As per this SR , we need to add the other amount entered in the ILRF setup for this program period
		to adj_prior_yr_amt field fo PREM_ADJ_LOS_REIM_FUND_POST table for the reserves there by allowing
		the user to adjusts the posting amount
        *********************************************************************************************/
		--First we need retrieve the other amount for this program period from the ilrf setup
		select @ilrf_other_amt=incur_los_reim_fund_othr_amt
		from PREM_ADJ_PGM_SETUP
		...
		update dbo.[PREM_ADJ_LOS_REIM_FUND_POST]
		set adj_prior_yy_amt = isnull(adj_prior_yy_amt,0) + isnull(@ilrf_other_amt,0)
		where [prem_adj_perd_id] = @premium_adj_period_id
		and [prem_adj_id] = @premium_adjustment_id
		and [custmr_id] = @customer_id
		and recv_typ_id = 71 -- lookup type: RECEIVABLE TYPE, lookup value: Reserve
        ...
        *********************************************************************************************/
		...
		update [dbo].[PREM_ADJ_LOS_REIM_FUND_POST]
		set lim_amt = round(isnull(curr_amt,0) + isnull(aggr_amt,0),0), -- Limited amount
		...
		* ILRF output header
		**********************/
		select @tot_amt_rounded=round(sum(isnull(tot_amt,0)),0)
		from PREM_ADj_PARMET_DTL 
		where prem_adj_id=@premium_adjustment_id 
		and prem_adj_perd_id=@premium_adj_period_id
		
		select @curr_amt_rounded=round(sum(isnull(curr_amt,0)),0) 
		from PREM_ADJ_LOS_REIM_FUND_POST
		where [prem_adj_perd_id] = @premium_adj_period_id
		and [prem_adj_id] = @premium_adjustment_id
		and [custmr_id] = @customer_id
		
		select @lim_amt_rounded=round(sum(isnull(lim_amt,0)),0) 
		from PREM_ADJ_LOS_REIM_FUND_POST
		where [prem_adj_perd_id] = @premium_adj_period_id
		and [prem_adj_id] = @premium_adjustment_id
		and [custmr_id] = @customer_id
		...
		IF(@tot_amt_rounded<>@curr_amt_rounded)
		BEGIN
		
		SET @diff_rounded=@tot_amt_rounded-@curr_amt_rounded
		
		--update dbo.[PREM_ADJ_LOS_REIM_FUND_POST]
		--	set curr_amt = curr_amt+@diff_rounded,
		--		lim_amt =  lim_amt+@diff_rounded,
		--		post_amt=  post_amt+@diff_rounded
		--	from dbo.[PREM_ADJ_LOS_REIM_FUND_POST] as a
		--	join 
		--	(
		--		select 
		...
		--		and [prem_adj_id] = @premium_adjustment_id
		--		and [custmr_id] = @customer_id
		--		and [recv_typ_id] in (31,29,30,34,32,33)
		--		and curr_amt=(select MAX(curr_amt)from dbo.[PREM_ADJ_LOS_REIM_FUND_POST]
		--		WHERE [prem_adj_perd_id] = @premium_adj_period_id
		--		and [prem_adj_id] = @premium_adjustment_id
		--		and [custmr_id] = @customer_id
		--		and [recv_typ_id] in (31,29,30,34,32,33))
		...
		********************************************/
		...
		set incur_los_reim_fund_amt = 
			...
			(
				select 
				sum( isnull(pdc.tot_amt,0) ) --as CHF
				from 
				dbo.PREM_ADJ_PARMET_SETUP psc 
				inner join dbo.PREM_ADJ_PARMET_DTL pdc on psc.prem_adj_parmet_setup_id=pdc.prem_adj_parmet_setup_id
				inner join #ILRF_Policy lp on lp.coml_agmt_id=pdc.coml_agmt_id
				where psc.prem_adj_id=@premium_adjustment_id 
				and psc.adj_parmet_typ_id=398
				and psc.prem_adj_perd_id=@premium_adj_period_id
			),
			tot_amt = 
			(
