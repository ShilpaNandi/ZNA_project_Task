...
BEGIN
  ...
  BEGIN
	  SELECT @WHERECONDITION = 
	  CASE WHEN (CHARINDEX('%',@AccountName))=0 
	  THEN ' WHERE CUSTMR.full_nm=''' + @ACCOUNTNAME + ''''
	  ELSE ' WHERE CUSTMR.full_nm LIKE ''' + @ACCOUNTNAME + ''''
	  END
 ...
 END 
   ...
   BEGIN
		SELECT @WHERECONDITION = 
		CASE WHEN (CHARINDEX('%',@AccountName))=0 
		THEN ' AND CUSTMR.full_nm=''' + @ACCOUNTNAME + ''''
		ELSE ' AND CUSTMR.full_nm LIKE ''' + @ACCOUNTNAME + ''''
		END
   END 
...
BEGIN
 ...
 IF PATINDEX('%WHERE%',@WhereCondition) = 0 
    BEGIN
		  SELECT @WHERECONDITION = 
		  CASE WHEN (CHARINDEX('%',@BROKER))=0 
		  THEN @WHERECONDITION + ' WHERE EXTRNL_ORG.full_name =''' + @BROKER + ''''
		  ELSE @WHERECONDITION + ' WHERE EXTRNL_ORG.full_name LIKE ''' + @BROKER + ''''
		  END
 ...
 ELSE
    BEGIN
		  SELECT @WHERECONDITION = 
		  CASE WHEN (CHARINDEX('%',@BROKER))=0 
		  THEN @WHERECONDITION + ' AND EXTRNL_ORG.full_name=''' + @BROKER + ''''
		  ELSE @WHERECONDITION + ' AND EXTRNL_ORG.full_name LIKE ''' + @BROKER + ''''
		  END
