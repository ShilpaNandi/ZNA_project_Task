...
namespace ZurichNA.AIS.DAL.Logic
{
    ...
    public class LSICustomersDA : DataAccessor<LSI_CUSTMR, LSICustomerBE, AISDatabaseLINQDataContext>
    {
            ...
            IEnumerable<LSIAllCustomersBE> query =
                (from cdd in this.Context.GetAIS_LSIAccount_Data()
                 select new LSIAllCustomersBE()
                 {
                     ...
                     ACCT_TYPE=cdd.AccountType,
                     ACCT_TYPE_DESC=cdd.AccountTypeDesc
        ...
        public IList<LSICustomerBE> getRelatedLSIAccountData(int AccountID)
        {
            ...
            (from cdd in this.Context.LSI_CUSTMRs
             ...
             select new LSICustomerBE()
             {
                 ...
                 ACCT_TYP = cdd.acct_typ,
                 CHF_IND = cdd.chf_ind == null ? false : cdd.chf_ind ,
                 PLB_IND = cdd.plb_ind == null ? false : cdd.plb_ind
             });
        ...
        public bool getDuplLSIPrimaryAccount(bool isActive, bool Primselected, int AccountID, int lsiAcctID)
        {
            if (this.Context == null)
                this.Initialize();
            IQueryable<LSICustomerBE> query = null;
            if (lsiAcctID > 0)
            {
                query =
              (from cdd in this.Context.LSI_CUSTMRs
               where cdd.prim_ind == true && cdd.actv_ind == true && cdd.custmr_id == AccountID && cdd.lsi_acct_id != lsiAcctID
               select new LSICustomerBE
               {
                   LSI_CUSTMR_ID = cdd.lsi_custmr_id
               });
        ...
        /// <summary>
        /// This method is used to verify the LSI primary account assigned while enabling the LSI account with Primary Indicator.
        /// Bug fix 12669
        /// </summary>
        /// <param name="AccountID"></param>
        /// <returns></returns>
        public bool CheckActiveLSIPrimaryAccount(int AccountID)
        {
            if (this.Context == null)
                this.Initialize();
        ...
        /// <param name="AccountID"></param>
        /// <returns></returns>
        public IList<LSICustomerBE> GetActiveLSIPrimaryAccounts(int AccountID,int LSIAccountID)
        {
            IList<LSICustomerBE> result = new List<LSICustomerBE>();
            if (this.Context == null)
                this.Initialize();
            ...
            IQueryable<LSICustomerBE> query =
              (from cdd in this.Context.LSI_CUSTMRs
               where cdd.prim_ind == true && cdd.actv_ind == true && cdd.custmr_id == AccountID && cdd.lsi_acct_id != LSIAccountID
               select new LSICustomerBE
               {
                   LSI_CUSTMR_ID = cdd.lsi_custmr_id
               });
        ...
        public bool isLSIRelatedAcctExist(int AccountID)
        {
            bool result = false;
            ...
            int count =
            (from cdd in this.Context.LSI_CUSTMRs
             where cdd.custmr_id == AccountID && cdd.actv_ind == true && cdd.chf_ind == true
             select new LSICustomerBE()
             {
                 LSI_ACCT_ID = cdd.lsi_acct_id
             }).Count();
