...
namespace ZurichNA.AIS.DAL.Logic
{
    public class PremAdjustmentDA : DataAccessor<PREM_ADJ, PremiumAdjustmentBE, AISDatabaseLINQDataContext>
    {
        ...
        public IList<PremiumAdjustmentBE> GetEMailInfo(int intAdjNo)
        {
            ...
            (from prmadj in this.Context.PREM_ADJs
             ...
             select new PremiumAdjustmentBE()
             {
                 ...
                 BP_NUMBER=prmadj.CUSTMR.finc_pty_id,
            ...
            if (count == 1)
            {
                persQuery =
                  (from cust in this.Context.CUSTMR_PERS_RELs
                   where cust.custmr_id == custmrID && cust.rol_id == role_id1 && cust.PER.actv_ind.Value == true && cust.PER.pers_id != null
                   select cust.PER.email_txt);
            }
            else if (count == 2)
            {
                persQuery =
                  (from cust in this.Context.CUSTMR_PERS_RELs
                   ...
                   && cust.PER.email_txt != "" && cust.PER.actv_ind.Value == true && cust.PER.pers_id != null
                   select cust.PER.email_txt);
            }
            else if (count == 3)
            {
                persQuery =
                  (from cust in this.Context.CUSTMR_PERS_RELs
                   where cust.custmr_id == custmrID && (cust.rol_id == role_id1 || cust.rol_id == role_id2 || cust.rol_id == role_id3)
                    && cust.PER.actv_ind.Value == true && cust.PER.pers_id != null
                   select cust.PER.email_txt);
            }
            else if (count == 4)
            {
       ...
       /// </summary>
        ...
        //Get recent valuation date
        public PremiumAdjustmentBE GetRecentValDate(int accountID)
        {
            if (this.Context == null)
                this.Initialize();

            IQueryable<PremiumAdjustmentBE> premiumAdjustmentBE
               = (from premadj in this.Context.PREM_ADJs
                  where (premadj.adj_sts_typ_id == 349 || premadj.adj_sts_typ_id == 352) && premadj.reg_custmr_id == accountID && premadj.valn_dt != null
                  select new PremiumAdjustmentBE
                {
                    PREMIUM_ADJ_ID = premadj.prem_adj_id,
            ...
            return premiumAdjustmentBE.OrderByDescending(p => p.PREMIUM_ADJ_ID).FirstOrDefault();


        }

        //Get recent valuation date
        public PremiumAdjustmentBE GetRecentAdjID(int accountID)
        {
            if (this.Context == null)
                this.Initialize();
