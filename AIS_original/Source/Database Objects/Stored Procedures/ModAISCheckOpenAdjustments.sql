...
if exists (select 1 from sysobjects 
		where name = 'ModAISCheckOpenAdjustments' and type = 'P')
	drop procedure ModAISCheckOpenAdjustments
go
...
---------------------------------------------------------------------
-----
-----	Proc Name:	ModAISCheckOpenAdjustments
-----
...
CREATE procedure [dbo].[ModAISCheckOpenAdjustments] 
@check_calc_prog_perds varchar(4000),
@check_recalc_prem_adj_perds varchar(4000),
@err_msg_op varchar(2000) output,
@calc_prog_perds_from_driver varchar(4000) output,
@recalc_prem_adj_perds_from_driver varchar(4000) output,
@open_adjustments_from_driver varchar(4000) output,
@create_user_id int,
@debug bit = 0
as
			...
			begin
				begin
					...
					if @cnt > 0
							...
							begin
									--Add prem_adj_pgm_id to @calc_prog_perds_from_driver
								if @debug = 1
								begin
								print '@calc_prog_perds_from_driver:' + convert(varchar(20),@calc_prog_perds_from_driver)
								end
								if(@calc_prog_perds_from_driver is null)
									begin
									set	@calc_prog_perds_from_driver=convert(varchar(20),@prem_adj_pgm_id) 
									end
								else
									begin
									set	@calc_prog_perds_from_driver=@calc_prog_perds_from_driver+','+convert(varchar(20),@prem_adj_pgm_id)
									end
					...
					print '@prem_adj_perd_id:' + convert(varchar(20),@prem_adj_perd_id)
						...
						begin
							select 
								...
								if(@open_adjustments_from_driver is null)
									begin
									set	@open_adjustments_from_driver=convert(varchar(20),@open_adj_id) 
									end
								else
									begin
									set	@open_adjustments_from_driver=@open_adjustments_from_driver+','+convert(varchar(20),@open_adj_id)
									end
							...
							begin
									--Add prem_adj_pgm_id to @recalc_prem_adj_perds_from_driver
								if @debug = 1
								begin
								print '@recalc_prem_adj_perds_from_driver:' + convert(varchar(20),@recalc_prem_adj_perds_from_driver)
								end
								if(@recalc_prem_adj_perds_from_driver is null)
									begin
									set	@recalc_prem_adj_perds_from_driver=convert(varchar(20),@prem_adj_perd_id) 
									end
								else
									begin
									set	@recalc_prem_adj_perds_from_driver=@recalc_prem_adj_perds_from_driver+','+convert(varchar(20),@prem_adj_perd_id)
									end
							end
...
if object_id('ModAISCheckOpenAdjustments') is not null
	print 'Created Procedure ModAISCheckOpenAdjustments'
else
	print 'Failed Creating Procedure ModAISCheckOpenAdjustments'
go

if object_id('ModAISCheckOpenAdjustments') is not null
	grant exec on ModAISCheckOpenAdjustments to public
go					