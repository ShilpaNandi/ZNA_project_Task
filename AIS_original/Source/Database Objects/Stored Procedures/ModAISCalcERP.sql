...
-----               06/05/2010 Venkat Kolimi
-----                Fixed  the issue 11659 i.e. Discrepancy between adjustment and Aries Posting Details
-----               05/21/2010 Venkat Kolimi
-----                Fixed  the issue 11712 i.e. prior ERP field used in the invc_amt calculation is chnaged.
-----               07/19/2010 Venkat Kolimi
-----                Fixed  the issue 11548 i.e. Calculation of LDR and Premuim Tax when there is LCF Aggregate Cap amount which is less than 
-----				 the actaul calculated amount 
-----				 09/26/2010 Venkat Kolimi
-----				 Fixed the issue 12844 i.e Incorrect LDF when Incl.LDF/IBNR indicator is set and ALAE is of any pro-rata.
-----				 04/20/2011 Venkat Kolimi
-----				 Fixed the issue 13909 i.e ERP is Less than Max but Calc is picking up Max in invoice
	...
	begin
		...
		begin
			...
			begin
				...
				set @sub_formulas = @sub_formulas + '   if ((@has_lcf = 1) and (@aloc_los_adj_exps_typ_id not in(78,79,80)))'
				...
				set @sub_formulas = @sub_formulas + '   else if(@has_lcf = 1)'
				set @sub_formulas = @sub_formulas + '	   select @ibnr_ldf_result = @ibnr_ldf_result - (@exc_ldf_ibnr_amt*@lcf)'
				...
				set @ldf_sub_formula = @ldf_sub_formula + '   if ((@has_lcf = 1) and (@aloc_los_adj_exps_typ_id not in(78,79,80)) )'
				...
				set @ldf_sub_formula = @ldf_sub_formula + '   else if (@has_lcf = 1)'
				set @ldf_sub_formula = @ldf_sub_formula + '	   select @ibnr_ldf_result = @ibnr_ldf_result - (@exc_ldf_ibnr_amt*@lcf)'
...
--				else 
	...
	set @decl_str = @decl_str + '@basic decimal(21,8),'
	...
	set @stmt = @stmt + ' ,@inc_erp_amt  decimal(15,2), @paid_erp_amt  decimal(15,2), @cfb_amt decimal(15,2), @has_ldf bit ,@has_ibnr bit, @has_lcf bit , @prem_adj_retro_id int,@prem_adj_retro_dtl_id int '
	...
	set @stmt = @stmt + ' ,@adj_cfb_diff decimal(15,2),@adj_cfb_minmax decimal(15,2),@is_not_master smallint,@std_sub_prem_amount decimal(15,2),@lcf_aggr_cap_amt decimal(15,2),@prem_adj_valn_dt datetime,@aloc_los_adj_exps_typ_id int,'
	...
	set @stmt = @stmt + ' select @has_ibnr = val from #params where id = 25'
	...
	set @stmt = @stmt + ' select @aloc_los_adj_exps_typ_id = aloc_los_adj_exps_typ_id from dbo.COML_AGMT where coml_agmt_id = @com_agm_id'
	...
	set @stmt = @stmt + '  if ((@has_lcf = 1) and (@aloc_los_adj_exps_typ_id not in(78,79,80)))'
	set @stmt = @stmt + '	select @inc_erp_amt = @inc_erp_amt - ((@exc_ldf_ibnr_amt + @subj_ldf_ibnr_amt)*@lcf - @subj_ldf_ibnr_amt)*(@tm)'
	set @stmt = @stmt + '  else if (@has_lcf = 1)'
	set @stmt = @stmt + '	select @inc_erp_amt = @inc_erp_amt - ((@exc_ldf_ibnr_amt)*@lcf)*(@tm)'
	set @stmt = @stmt + '  else'
	...
	set @stmt = @stmt + '  else if (@has_lcf = 1)'
	set @stmt = @stmt + '	select @paid_erp_amt = @paid_erp_amt - ((@exc_ldf_ibnr_amt)*@lcf*@paid_ratio)*(@tm)'
	set @stmt = @stmt + '  else'
	...
	set @stmt = @stmt + '  else if (@has_lcf = 1)'
	set @stmt = @stmt + '	select @tm_result = @tm_result - ((@exc_ldf_ibnr_amt)*@lcf)*(@tm - 1)'
	set @stmt = @stmt + '  else'
...
--		end
		--11548 Fix
		if (@lcf_aggr_cap_set_pgm_amt <>0 and @recalc_lcf=1) --Tax
		begin
			set @stmt = @stmt + ' set @lcf_aggr_cap = 0'
			set @stmt = @stmt + '  if (@has_lcf = 1)'
			set @stmt = @stmt + ' begin'
			set @stmt = @stmt + '  if (@has_ldf = 1)'
			set @stmt = @stmt + ' begin'
			set @stmt = @stmt + '		select @tm_result = @tm_result -((isnull(@lcf_aggr_cap,0))*(@ldf)*(@tm-1))'
			set @stmt = @stmt + ' end'
			set @stmt = @stmt + '  else if (@has_ibnr = 1)'
			set @stmt = @stmt + ' begin'
			set @stmt = @stmt + '	select @tm_result = @tm_result -((isnull(@lcf_aggr_cap,0))*(@ibnr)*(@tm-1))'
			set @stmt = @stmt + ' end'
			set @stmt = @stmt + '  else '
			set @stmt = @stmt + ' begin'
			set @stmt = @stmt + '	select @tm_result = @tm_result -((isnull(@lcf_aggr_cap,0))*(@tm-1))'
			set @stmt = @stmt + ' end'
			set @stmt = @stmt + ' end'
	...
	if (@recalc_lcf=1) --Tax
		begin
			...
			set @stmt = @stmt + '  if (@has_ldf = 1)'
			set @stmt = @stmt + ' begin'
			set @stmt = @stmt + '	set @ibnr_ldf_result = @ibnr_ldf_result -(isnull(@lcf_aggr_cap,0)*(@ldf-1)) '
			set @stmt = @stmt + ' end'
			set @stmt = @stmt + '  else if (@has_ibnr = 1)'
			set @stmt = @stmt + ' begin'
			set @stmt = @stmt + '	set @ibnr_ldf_result = @ibnr_ldf_result -(isnull(@lcf_aggr_cap,0)*(@ibnr-1)) '
			set @stmt = @stmt + ' end'
...
--isnull(sum(d.adj_incur_ernd_retro_prem_amt),0)
		...
		******************************************************************/
		...
		--11548 Fix
		if (@recalc_lcf = 1)
			select @stmt = replace(@stmt, 'adj_incur_ernd_retro_prem_amt = @inc_erp_amt,adj_paid_ernd_retro_prem_amt = @paid_erp_amt',' los_conv_fctr_amt = @lcf_result,los_dev_resrv_amt=@ibnr_ldf_result')
		else
...
--				)
		...
		set @erp_applies = 0
		/*************************************************************************************
		* Apply Maximum / Minimum rules in case of LCF aggregate cap amount 
		**********************************************************************************/
		if ((@paid_incur_type = 298) or (@paid_incur_type = 297)) -- PAID and INCURRED Program Types
		begin
			...
			*******************************************/
			...
			if @debug = 1
			begin
			print '@minmax_incurred_erp_by_pp_amt_lcf_agg: ' + convert(varchar(20),@minmax_incurred_erp_by_pp_amt)
			print '@minmax_paid_erp_by_pp_amt_lcf_agg: ' + convert(varchar(20),@minmax_paid_erp_by_pp_amt)
			print '@minmax_cfb_by_pp_amt_lcf_agg: ' + convert(varchar(20),@minmax_cfb_by_pp_amt)
			end
			if (			
					((select no_lim_ind 
					from dbo.PREM_ADJ_PGM_RETRO
		...
		**************************************************/
		...
		@perd_erp_amt = case when @perd_code = 'ERP' then isnull(sum( ADJ_INCUR_ERND_RETRO_PREM_AMT ),0) + isnull(sum( adj_dedtbl_wrk_comp_los_amt ),0) else isnull(sum( ADJ_INCUR_ERND_RETRO_PREM_AMT),0)  end
		from dbo.PREM_ADJ_RETRO_DTL
		where prem_adj_id = @premium_adjustment_id 
		and prem_adj_perd_id = @premium_adj_period_id 
		...
		***********************************************/
		...
		------------------------------------end: 11283 Bug Fix--------------------------------------------------
			...
			--------begin: changes for bug # 10351 and modification for 11283 Bug---------------
		...
		select 
		dbo.fn_GetRetroId(@premium_adj_prog_id,@premium_adjustment_id,@premium_adj_period_id,coml_agmt_id),
		@premium_adj_period_id,
		@premium_adjustment_id,
		@customer_id,
		@premium_adj_prog_id,
		...
		and dbo.fn_GetRetroId(@premium_adj_prog_id,@premium_adjustment_id,@premium_adj_period_id,coml_agmt_id) is not null
		--------end: changes for bug # 10351 and modification for bug 11283------------
		...
		**************************/
		...
		inner join dbo.PREM_ADJ_RETRO op on (prd.prem_adj_id = op.prem_adj_id ) and (prd.prem_adj_pgm_id = op.prem_adj_pgm_id )
		where pa.adj_sts_typ_id in (349,352) -- Lookup IDs for applicable statuses: Final Invoice and Transmitted
...
--		begin
			...
			(
				...
				h.prem_adj_pgm_id,
				h.coml_agmt_id,
				sum(isnull(d.cesar_cd_tot_amt,0)) as sum_erp
				from dbo.PREM_ADJ_RETRO_DTL d
				inner join dbo.PREM_ADJ_RETRO h on (d.prem_adj_retro_id = h.prem_adj_retro_id) and (d.prem_adj_perd_id = h.prem_adj_perd_id) and (d.prem_adj_id = h.prem_adj_id) and (d.custmr_id = h.custmr_id)
			...
			inner join dbo.COML_AGMT ca on (ca.coml_agmt_id = r.coml_agmt_id) and (ca.prem_adj_pgm_id = r.prem_adj_pgm_id) and (ca.custmr_id = r.custmr_id) --and (ca.actv_ind = 1)--(11283 Bug Fix)
			...
			***********************************************/
			...
			inner join dbo.COML_AGMT ca on (ca.coml_agmt_id = h.coml_agmt_id) and (ca.prem_adj_pgm_id = h.prem_adj_pgm_id) and (ca.custmr_id = h.custmr_id) --and (ca.actv_ind = 1)--(11283 Bug Fix)
			where dd.prem_adj_perd_id = @premium_adj_period_id
			and dd.prem_adj_id = @premium_adjustment_id
			and dd.custmr_id = @customer_id
			and dd.prem_adj_pgm_id = @premium_adj_prog_id
...
----		and ca.adj_typ_id in(63,70) -- Adjustment type IDs for: 'Incurred DEP','Paid loss DEP'
		...
		--******** Start of Bug fix 11659 ********-------------
		--create table #tmpCeasrAmt
		--(
		--coml_agmt_id int,
		--prior_cesar_cd_tot_amt decimal(15,2),
		--current_cesar_cd_tot_amt decimal(15,2)
		--)


		--insert into #tmpCeasrAmt
		--(
		--coml_agmt_id,
		--current_cesar_cd_tot_amt
		--) 
		--(
		--select 
		--coml_agmt_id,
		--isnull(sum(cesar_cd_tot_amt),0) 
		--from prem_adj_retro_dtl 
		--where prem_adj_id=@premium_adjustment_id 
		--and prem_adj_pgm_id=@premium_adj_prog_id 
		--group by coml_agmt_id
		--)

		--update #tmpCeasrAmt
		--set prior_cesar_cd_tot_amt=b.sum_prior_cesar_cd_tot_amt
		--from #tmpCeasrAmt as a
		--join
		--(
		--select coml_agmt_id,
		--isnull(sum(cesar_cd_tot_amt),0) as sum_prior_cesar_cd_tot_amt
		--from prem_adj_retro_dtl
		--where
		...
		--group by coml_agmt_id
		--) as b
		--on a.coml_agmt_id=b.coml_agmt_id

		
		--declare @sum_aries_tot_amt_with_out_cesar decimal(15,2)

		--select @sum_aries_tot_amt_with_out_cesar=sum(aries_tot_amt) 
		--from prem_adj_retro
		--where prem_adj_id=@premium_adjustment_id
		--and prem_adj_pgm_id=@premium_adj_prog_id
		--and coml_agmt_id in(select coml_agmt_id from  #tmpCeasrAmt where ((prior_cesar_cd_tot_amt-current_cesar_cd_tot_amt)=0))

		--update dbo.PREM_ADJ_RETRO 
			--set aries_tot_amt = aries_tot_amt + isnull(@sum_aries_tot_amt_with_out_cesar,0)
		--where prem_adj_id = @premium_adjustment_id
		--and prem_adj_perd_id = @premium_adj_period_id
		--and prem_adj_pgm_id=@premium_adj_prog_id
		--and coml_agmt_id in
		--(
		--	select top 1 coml_agmt_id from  #tmpCeasrAmt where ((prior_cesar_cd_tot_amt-current_cesar_cd_tot_amt)<>0)
		--)


		--drop table #tmpCeasrAmt---
		
		--******** End of Bug fix 11659 ********-------------
