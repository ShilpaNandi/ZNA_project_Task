					...
					if @cnt > 0
								begin
									--retrieving open adjustment id on same val date
										...
										and substring(isnull(pa.fnl_invc_nbr_txt,' '),1,3)<>'RTV'
										and pg.prem_adj_pgm_id=@prem_adj_pgm_id
										set @err_message = 'A key field has been changed to one or more of the program periods. Please proceed to the Adjustment Review Menu_BU Broker Tab OR Cancel the adjustment and Calculate so changes will be included in the adjustment(s).' 
				...
				begin
					...
					print '@prem_adj_perd_id:' + convert(varchar(20),@prem_adj_perd_id)
						...
						begin
							...
							set @err_message = 'A key field has been changed to one or more of the program periods. Please proceed to the Adjustment Review Menu_BU Broker Tab OR Cancel the adjustment and Re-set the Broker and/or BU/Office to the prior values and calculate.' 
										+ ';Customer ID: ' + convert(varchar(20),@customer_id) 
										+ ';Currently open adjustment ID: ' + convert(varchar(20),@open_adj_id) 
										+ ';Program Period ID: ' + convert(varchar(20),@prem_adj_pgm_id) 
