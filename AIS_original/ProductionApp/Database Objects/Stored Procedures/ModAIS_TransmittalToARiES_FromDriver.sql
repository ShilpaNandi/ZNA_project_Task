...
IF EXISTS (SELECT 1 FROM sysobjects 
	WHERE NAME = 'ModAIS_TransmittalToARiES_FromDriver' and TYPE = 'P')
	DROP PROC ModAIS_TransmittalToARiES_FromDriver
GO
SET ANSI_NULLS ON
GO
...
---------------------------------------------------------------------
-----
-----	Proc Name:		ModAIS_TransmittalToARiES_FromDriver
-----
-----	Version:		SQL Server 2005
-----
-----	Created:		Anil Nandakumar 11/18/2008
-----
-----	Description:	This procedure is called from the calc driver. 
-----					It will populate the ARIES_TRNSMTL_HIST table with all the 
...
CREATE PROCEDURE [dbo].[ModAIS_TransmittalToARiES_FromDriver] 
@prem_adj_id int,
...
begin try
	...
	SET @nErrorCode = 0
		...
		INSERT INTO ARIES_TRNSMTL_HIST (zzrectyp_txt,aktyp_txt,gpart_txt,gpart_ext_txt,vtref_txt,posnr_txt,psngl_txt,
			...
			prem_adj_perd_id,prem_adj_id,custmr_id,post_trns_typ_id,adj_typ_txt,trnsmtl_sent_ind,Post_cd)
		SELECT 
		    CONVERT(char(1), '2')  , 		-- ZZRECTYPE (1)
			CONVERT(char(2), '01') , 		-- AKTYP(2)
			CASE WHEN ISNULL(CONVERT(BIGINT,LSI_CUSTMR.lsi_acct_id),0)=0  
				THEN CONVERT(char(10),CONVERT(BIGINT,finc_pty_id))  
			...
			CONVERT(char(8), getdate()),  	-- updt_dt
			CONVERT(char(1), 1),			-- crte_user_id
			CONVERT(char(8), getdate()),	-- crte_dt
	...
	WHERE PREM_ADJ_PARMET_SETUP.adj_parmet_typ_id=401
		...
		AND PREM_ADJ_PERD.prem_adj_perd_id = @prem_adj_perd_id
	...
	WHERE PREM_ADJ_PARMET_SETUP.adj_parmet_typ_id=401
		...
		SELECT 
		    CONVERT(char(1), '2')  , 		-- ZZRECTYPE (1)
			CONVERT(char(2), '01') , 		-- AKTYP(2)
			CASE WHEN ISNULL(CONVERT(BIGINT,LSI_CUSTMR.lsi_acct_id),0)=0  THEN CONVERT(char(10),CONVERT(BIGINT,finc_pty_id))  
			ELSE CONVERT(char(10), '') END  , 		-- GPART(3)
			CONVERT(char(20), '')  , 		-- GPART_EXT(4)
			CONVERT(char(20), '')  ,  		-- VTREF(5)
			CONVERT(char(12), '')  , 		-- POSNR(6)
	...
	WHERE PREM_ADJ_PARMET_SETUP.adj_parmet_typ_id=401
		...
		INSERT INTO ARIES_TRNSMTL_HIST (zzrectyp_txt,aktyp_txt,gpart_txt,gpart_ext_txt,vtref_txt,posnr_txt,psngl_txt,
			...
			custmr_id,post_trns_typ_id,	adj_typ_txt,trnsmtl_sent_ind,Post_cd)
		SELECT	
		    CONVERT(char(1), '2')  , 		-- ZZRECTYPE (1)
			CONVERT(char(2), '01') , 		-- AKTYP(2)
			...
			CONVERT(char(15), REPLACE(REPLACE(ISNULL(SPACE(15 - LEN(CONVERT(varchar(15), 
			CONVERT(bigint, PREM_ADJ_RETRO.invc_amt  * 100)))) +
			CONVERT(varchar(15), CONVERT(bigint, PREM_ADJ_RETRO.invc_amt  * 100)), 
			'000000000000000'), ' ', '0'), '-', '0')) + 
			CASE WHEN (@Reverse*PREM_ADJ_RETRO.invc_amt  < 0)  THEN '-' ELSE ' ' END , -- AMOUNT_TOTAL(22)
	...
	WHERE PREM_ADJ_PARMET_SETUP.adj_parmet_typ_id=400 -- ILRF
		...
		SELECT	
			...
			CONVERT(char(15), REPLACE(REPLACE(ISNULL(SPACE(15 - LEN(CONVERT(varchar(15), 
			CONVERT(bigint, SUM(PREM_ADJ_RETRO.invc_amt)  * 100)))) +
			CONVERT(varchar(15), CONVERT(bigint, SUM(PREM_ADJ_RETRO.invc_amt)  * 100)), 
			'000000000000000'), ' ', '0'), '-', '0')) + 
			CASE WHEN (@Reverse*SUM(PREM_ADJ_RETRO.invc_amt)  < 0)  THEN '-' ELSE ' ' END , -- AMOUNT_TOTAL(22)
		...
		HAVING SUM(PREM_ADJ_RETRO.invc_amt) <> 0
...
if object_id('ModAIS_TransmittalToARiES_FromDriver') is not null
	print 'Created Procedure ModAIS_TransmittalToARiES_FromDriver'
else
	print 'Failed Creating Procedure ModAIS_TransmittalToARiES_FromDriver'
go

if object_id('ModAIS_TransmittalToARiES_FromDriver') is not null
	grant exec on ModAIS_TransmittalToARiES_FromDriver to public
go
