	...
	begin
		...
		begin
			...
			begin
				...
				set @sub_formulas = @sub_formulas + '	select @exc_ldf_ibnr_amt = sum(isnull(exc_ldf_ibnr_amt,0))  from ARMIS_LOS_POL where prem_adj_pgm_id = @premium_adj_prog_id and coml_agmt_id = @com_agm_id and st_id = @state_id and ((prem_adj_id is null) or (prem_adj_id = @premium_adjustment_id)) '
				set @sub_formulas = @sub_formulas + '	select @subj_ldf_ibnr_amt = sum(isnull(subj_ldf_ibnr_amt,0))  from ARMIS_LOS_POL where prem_adj_pgm_id = @premium_adj_prog_id and coml_agmt_id = @com_agm_id and st_id = @state_id and ((prem_adj_id is null) or (prem_adj_id = @premium_adjustment_id)) '
				...
				set @ldf_sub_formula = @ldf_sub_formula + '		select @exc_ldf_ibnr_amt = sum(isnull(exc_ldf_ibnr_amt,0))  from ARMIS_LOS_POL where prem_adj_pgm_id = @premium_adj_prog_id and coml_agmt_id = @com_agm_id and st_id = @state_id and ((prem_adj_id is null) or (prem_adj_id = @premium_adjustment_id)) '
				set @ldf_sub_formula = @ldf_sub_formula + '	select @subj_ldf_ibnr_amt = sum(isnull(subj_ldf_ibnr_amt,0))  from ARMIS_LOS_POL where prem_adj_pgm_id = @premium_adj_prog_id and coml_agmt_id = @com_agm_id and st_id = @state_id and ((prem_adj_id is null) or (prem_adj_id = @premium_adjustment_id)) '
...
--				else 
	...
	set @stmt = @stmt + ' ,@adj_cfb_diff decimal(15,2),@adj_cfb_minmax decimal(15,2),@is_not_master smallint,@std_sub_prem_amount decimal(15,2),'
	...
	set @stmt = @stmt + ' begin'
	set @stmt = @stmt + '	select @exc_ldf_ibnr_amt = sum(isnull(exc_ldf_ibnr_amt,0)), @subj_ldf_ibnr_amt =  sum(isnull(subj_ldf_ibnr_amt,0)) from ARMIS_LOS_POL where prem_adj_pgm_id = @premium_adj_prog_id and coml_agmt_id = @com_agm_id and st_id = @state_id and ((prem_adj_id is null) or (prem_adj_id = @premium_adjustment_id)) '
	...
	begin
		set @err_message = 'ERP: Audit does not exist for the following policies: '  + @policy_str  +  '; For customer ID: ' + convert(varchar(20),@customer_id) + ';Program Period ID: '  + convert(varchar(20),@premium_adj_prog_id)
        if (@pgm_type = 1 ) --DEP Program Type
           begin
		    rollback transaction ModAISCalcERP
           end
	...
	declare erp_base_cur cursor LOCAL FAST_FORWARD READ_ONLY 
		...
		set @updt_stmt = ' update dbo.PREM_ADJ_RETRO_DTL WITH (ROWLOCK) set adj_incur_ernd_retro_prem_amt = @inc_erp_amt,adj_paid_ernd_retro_prem_amt = @paid_erp_amt,prem_tax_amt = @tm_result,'
...
--				)
		...
		**************************************************/
		...
		set ernd_retro_prem_min_amt = @perd_erp_min_amt,
			...
			ernd_retro_prem_amt = @perd_erp_amt,
...
--		begin
			...
			***********************************************/
			...
			update dbo.PREM_ADJ_RETRO WITH (ROWLOCK)
			set post_idnmty_amt = isnull(tm.sum_subj_paid_idnmty_amt,0) - isnull(tm.sum_prev_subj_paid_idnmty_amt,0),
				post_resrv_idnmty_amt = isnull(tm.sum_subj_resrv_idnmty_amt,0) - isnull(tm.sum_prev_subj_resrv_idnmty_amt,0)
			...
			(
				...
				sum(isnull(prev_subj_paid_idnmty_amt,0)) as sum_prev_subj_paid_idnmty_amt,
				sum(isnull(subj_resrv_idnmty_amt,0)) as sum_subj_resrv_idnmty_amt,
				sum(isnull(prev_subj_resrv_idnmty_amt,0)) as sum_prev_subj_resrv_idnmty_amt
				from dbo.PREM_ADJ_RETRO_DTL d
		...
		begin
			update dbo.PREM_ADJ_RETRO WITH (ROWLOCK)
			set paid_los_bil_amt = isnull(tm.total_plb_amt,0)
			from dbo.PREM_ADJ_RETRO as r
			join 
			(
				select
...
--				--and adj_typ_id not in (63,70) -- Adjustment types for: 'Incurred DEP','Paid Loss DEP'
			...
			(
				...
				case when @perd_code = 'ERP' then sum(isnull(d.cash_flw_ben_amt,0)) 
					else -sum(isnull(d.adj_cash_flw_ben_amt,0)) end as sum_cfb_amt,
...
--				case when @perd_code = 'ERP' then sum(isnull(adj_dedtbl_wrk_comp_los_amt,0)) else 0 end as sum_adj_ded_wc_loss_amt,
				...
				case when @perd_code = 'ERP' then sum(isnull(d.cash_flw_ben_amt,0)) else -sum(isnull(d.adj_cash_flw_ben_amt,0)) end as sum_cfb_amt,
				sum(isnull(prior_cash_flw_ben_amt,0)) as sum_prior_cfb_amt
				from dbo.PREM_ADJ_RETRO_DTL d
				inner join dbo.PREM_ADJ_RETRO ih
				on (d.prem_adj_retro_id = ih.prem_adj_retro_id) and (d.prem_adj_perd_id = ih.prem_adj_perd_id) and (d.prem_adj_id = ih.prem_adj_id) and (d.custmr_id = ih.custmr_id)
