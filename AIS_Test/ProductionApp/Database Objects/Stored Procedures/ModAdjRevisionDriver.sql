...
if exists (select 1 from sysobjects 
                where name = 'ModAdjRevisionDriver' and type = 'P')
...
begin
	...
	--begin		
		...
		from
				...
				and prem_adj_pgm_id in (select prem_adj_pgm_id from prem_adj_perd where PREM_ADJ_ID = @prem_adj_id )
					group by prem_adj_id, prem_adj_pgm_id, reg_custmr_id  
				) as t1
				...
				where	
					...
					and	  pa.adj_sts_typ_id in (349,352)
		) as t3
		inner join PREM_ADJ_PGM pag on ( t3.prem_adj_pgm_id = pag.prem_adj_pgm_id)
		and pag.actv_ind = 1
	...
	-- step 11. Invoke the Calc Engine Business Service for that adjustment’s Premium adjustment Period records as a recalculation
