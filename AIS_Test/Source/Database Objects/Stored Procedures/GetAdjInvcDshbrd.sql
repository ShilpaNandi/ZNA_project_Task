...
prem_adj_pgm_sts_id,
pgm_perd_sts_typ_id,
(select CASE when (cust.thrd_pty_admin_funded_ind<>1) then count(LSI.Custmr_id)ELSE 1 END from LSI_Custmr LSI where LSI.Custmr_id=pag.Custmr_id) as LSI_Custmr_Count,
...
and pag.nxt_valn_dt <= 
case when pag.fnl_adj_dt is not null then 
pag.fnl_adj_dt else  dateadd(mm,1,pag.nxt_valn_dt) end
