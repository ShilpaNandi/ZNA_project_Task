IF EXISTS (SELECT 1 FROM sysobjects 
	WHERE NAME = 'ModAIS_Process_Mass_Reassignments_Uploads' and TYPE = 'P')
	DROP PROC ModAIS_Process_Mass_Reassignments_Uploads
GO
SET ANSI_NULLS OFF
GO
...
CREATE PROCEDURE [dbo].[ModAIS_Process_Mass_Reassignments_Uploads]               
...
BEGIN              
 ...
 begin              
  ...
  CREATE TABLE #CUSTID_FINC_PTY_ID             
   (            
   ID INT IDENTITY(1,1),   
   CUSTID varchar(max)  
   )               
 ...
 begin              
  begin              
   ...
   begin --Blank data check : Failed              
    ...
    IF(ISNULL(@validation_message_blank,'')<>'' OR ISNULL(@validation_message_user,'')<>'' OR ISNULL(@validation_message_Exist,'')<>'')
		 ...
		 BEGIN 
				   ...
				   if @debug = 1              
					begin              
					 ...
					 print' @intcustmr_id: ' + convert(varchar(MAX), @acct_id)               
					end 
 ...
 IF(@Validate=0 AND ISNULL(@finc_pty_id,'')<>'')  
   ...
   TRUNCATE TABLE #CUSTID_FINC_PTY_ID
   ...
   WHILE (@CNT>=@I)            
    ...
    SET @intcustmr_BP_id=NULL
    SET @intcustmr_BP_id=(SELECT CUSTID FROM #CUSTID_FINC_PTY_ID where ID=@I)  
    
    if @debug = 1              
    begin              
     print 'While loop for the BP Number :'     
     print' @I: ' + convert(varchar(MAX), @I)            
     print' @intcustmr_BP_id: ' + convert(varchar(MAX), @intcustmr_BP_id)               
     print' @rol_id: ' + convert(varchar(MAX), @rol_id)  
     print' @intPersID: ' + convert(varchar(MAX), @intPersID)  
    end   
      
      IF((SELECT COUNT(*) FROM [CUSTMR_PERS_REL] where custmr_id=@intcustmr_BP_id and rol_id=@rol_id)=0)        
      BEGIN     
      ...
      INSERT INTO [CUSTMR_PERS_REL](custmr_id,pers_id,rol_id,crte_dt,crte_user_id)        
      SELECT @intcustmr_BP_id,@intPersID,@rol_id,GETDATE(),ISNULL(@intUserID,9999)     
       if @debug = 1   
       begin     
       print convert(varchar(MAX),@rol_id)+' role is created successfully for account id '+convert(varchar(MAX),@intcustmr_BP_id)+' with person id '+ convert(varchar(MAX),@intPersID)+'.'    
       end  
      ...
      BEGIN       
        ...
        [updt_user_id]=ISNULL(@intUserID,9999),  
        [updt_dt]=GETDATE()   
        WHERE custmr_id=@intcustmr_BP_id 
        and  rol_id=@rol_id  
        
        if @debug = 1   
        begin     
        print convert(varchar(MAX),@rol_id)+'role is updated successfully for account id '+convert(varchar(MAX),@intcustmr_BP_id)+' with person id '+ convert(varchar(MAX),@intPersID)+'.'   
        end   
      END    
...
IF OBJECT_ID('ModAIS_Process_Mass_Reassignments_Uploads') IS NOT NULL
	PRINT 'CREATED PROCEDURE ModAIS_Process_Mass_Reassignments_Uploads'
ELSE
	PRINT 'FAILED CREATING PROCEDURE ModAIS_Process_Mass_Reassignments_Uploads'
GO

IF OBJECT_ID('ModAIS_Process_Mass_Reassignments_Uploads') IS NOT NULL
	GRANT EXEC ON ModAIS_Process_Mass_Reassignments_Uploads TO PUBLIC
GO