...
namespace ZurichNA.AIS.Business.Logic
{
    ...
    public class ProgramPeriodsBS : BusinessServicesBase<ProgramPeriodBE, ProgramPeriodDA>
    {
        ...
        public bool IsProgramPeriodExitsAndDisabled(int progPerID, int brokerID, int buOffice, DateTime StartDate, DateTime EndDate, int custID, DateTime valDate, int progType)
        {
            ...
            try
            {
                Results = proPreDA.IsProgramPeriodExitsAndDisabled(progPerID, brokerID, buOffice, StartDate, EndDate, custID, valDate, progType);
            }
            catch (Exception ex)
            {
                RetroDatabaseException myException = new RetroDatabaseException(ex.Message, ex);
        ...
        /// <param name="brokerID"></param>
        /// <returns></returns>
        public IList<ProgramPeriodBE> GetCustomerList(string customerids, int buID, int brokerID,int idType)
        {
            IList<ProgramPeriodBE> ProgramPrdBE;
            try
            {
                ProgramPrdBE = new ProgramPeriodDA().GetCustomerDetails(customerids, buID, brokerID, idType);
            }
            catch (Exception ex)
            {
                RetroDatabaseException myException = new RetroDatabaseException(ex.Message, ex);
        ...
        public string CalcDriver(int CustomerID, string strPRGIDs, string strPERDIDs, bool Flag, bool ILRFlag, int UserID,bool CHFFlag)
        {
            ProgramPeriodDA prgPRD = new ProgramPeriodDA();
            return prgPRD.CalcDriver(CustomerID, strPRGIDs, strPERDIDs, Flag, ILRFlag, UserID, CHFFlag);
        }
        ...
        /// <returns></returns>
        public IList<ProgramPeriodBE> GetQCProgramPeriods(int AccNo, int intAdjNo)
        {
            ProgramPeriodDA prgPRD = new ProgramPeriodDA();
            IList<ProgramPeriodBE> ProgramPrdSearchBE = new List<ProgramPeriodBE>();
            ProgramPrdSearchBE = prgPRD.GetQCProgramPeriods(AccNo, intAdjNo);
            return ProgramPrdSearchBE;
        ...
        ///-----------------------------------------------------------------------------------///
        /// <summary>
        /// Business service Method used to retrieve the Program Period Info for the Bu Broker Review Screen
        /// </summary>
        ...
        public IList<ProgramPeriodBE> GetBuBrokerReviewProgramPeriodList(int intPremAdjID)
        {
            ...
            try
            {
                programPrdBE = new ProgramPeriodDA().GetBuBrokerReviewProgramPeriodList(intPremAdjID);
            }
            catch (Exception ex)
            {
                RetroDatabaseException myException = new RetroDatabaseException(ex.Message, ex);
        ...
        {
            return (new ProgramPeriodDA()).GetILRFOtherAMounts(CustomerID,strPRGIDs,strPERDIDs);
        ...
        {
            return (new ProgramPeriodDA()).GetCusmrDetails_AccountAssgiment(custmr_id, custmr_ids, buofficeid, bp_ids, buname, buoffice, brokerid, roleid, userid, bpnumber, acct_range, buoffice_range, broker_range);
        }


        public int AddCustPersRelAccountAssignment(string strCustmrids, string strRoleids, string strPersids, int userid)
        {
            return (new ProgramPeriodDA()).AddCustPersRelAccountAssignment(strCustmrids, strRoleids, strPersids, userid);
        }
        ...
        {
            return (new ProgramPeriodDA()).AddCustPersRelAccountAssignmentWithUserName(strCustmrids, strPersids, strRoleids, userid, flag, vflag);
        }
        ...
        {
            return (new ProgramPeriodDA()).GetCustmrDetails_WithRoles(custmr_id, custmr_ids, buofficeid, bp_ids, buname, buoffice, brokerid, roleid, userid, bpnumber, acct_range, buoffice_range, broker_range);
        }

        public bool CheckCustmrIDExist(int custmrID)
        {
            return (new ProgramPeriodDA()).CheckCustmrIDExist(custmrID);
        }

        public bool CheckBPNumExist(string bpNumber)
        {
            return (new ProgramPeriodDA()).CheckBPNumExist(bpNumber);
        }

        public bool AccountMatchBulkCopy(DataTable dt)
        {
            return (new ProgramPeriodDA()).AccountMatchBulkCopy(dt);
        }

        public bool MassReassignStage(DataTable dt)
        {
            return (new ProgramPeriodDA()).MassReassignStage(dt);
        }

        public string ModAISProcessMassReassignments(string userID, DateTime dtUploadDateTime)
        {
            return (new ProgramPeriodDA()).ModAISProcessMassReassignments(userID, dtUploadDateTime);
        }

        public bool MultipleUserUploadStage(DataTable dt,int assignType)
        {
            return (new ProgramPeriodDA()).MultipleUserUploadStage(dt, assignType);
        }

        public string ModAISProcessMassReassignmentsUploads(string userID, DateTime dtUploadDateTime, int AssignType)
        {
            return (new ProgramPeriodDA()).ModAISProcessMassReassignmentsUploads(userID, dtUploadDateTime, AssignType);
        }

        public DataTable MassReassignmentsUploadsError(string userID, DateTime dtUploadDateTime, int assignType)
        {
            return (new ProgramPeriodDA()).MassReassignmentsUploadsError(userID, dtUploadDateTime, assignType);
        }

        //Method to call when Adjustment is Finalised to Post to CESAR
        public void ModAISCodingToCesar(int intPremAdjID)
        {
            try
            {
                (new ProgramPeriodDA()).ModAISCodingToCesar(intPremAdjID);
        ...
        public IList<ProgramPeriodSearchListBE> GetPGMByAdjIDValDate(int intPremAdjID, int custmrID,DateTime dtValDate)
        {
            IList<ProgramPeriodSearchListBE> ProgramPrdSearchBE;
            ProgramPrdSearchBE = new ProgramPeriodDA().GetPGMByAdjIDValDate(intPremAdjID, custmrID, dtValDate);
        ...
        public ProgramPeriodBE GetPrevValnDate(int PremAdjID)
        {
            return new ProgramPeriodDA().GetPrevValnDate(PremAdjID);
        }

        //written by siba for long running report - spread sheet

        public Dictionary<int, string> GetProgramPeriodTypes(List<int> prgPrdIDList)
        {
            IList<ProgramPeriodSearchListBE> ProgramPrdSearchBE;
            ProgramPrdSearchBE = new ProgramPeriodDA().GetProgramPeriodTypes(prgPrdIDList);
            Dictionary<int, string> prgPrdList = new Dictionary<int, string>();
            for (int row = 0; row < ProgramPrdSearchBE.Count; row++)
            {
                ProgramPrdSearchBE[row].STARTDATE_ENDDATE_PGMTYP = ProgramPrdSearchBE[row].STARTDATE_ENDDATE + " " + ProgramPrdSearchBE[row].PGMTYP;
                prgPrdList.Add(ProgramPrdSearchBE[row].PREM_ADJ_PGM_ID, ProgramPrdSearchBE[row].STARTDATE_ENDDATE_PGMTYP);
            }
            return prgPrdList;
        ...
        {
            return (new ProgramPeriodDA()).GetCHFReport(adjNO, flag, histFlag);
        }

        public DataTable GetAdjReport(int adjNO, int flag, bool histFlag, int cmtID)
        {
            return (new ProgramPeriodDA()).GetAdjReport(adjNO, flag, histFlag, cmtID);
        }
        //Phase-3
        public DataTable GetProgramSummaryPeriodDetails(int adjNO)
        {
            return (new ProgramPeriodDA()).GetProgramSummaryPeriodDetails(adjNO);
        }
        //Phase-3
        public DataTable GetProgramSummaryRetroDetails(int adjNO)
        {
            return (new ProgramPeriodDA()).GetProgramSummaryRetroDetails(adjNO);
        }
        //Phase-3
        public DataTable GetProgramSummaryILRFDetails(int adjNO, int flag, bool histFlag, int cmtcatGid)
        {
            return (new ProgramPeriodDA()).GetProgramSummaryILRFDetails(adjNO,flag,histFlag,cmtcatGid);
        }
        //Phase-3
        public DataTable GetProgramSummaryOtherAdjsutmentDetails(int adjNO, int flag, bool histFlag, int cmtcatGid)
        {
            return (new ProgramPeriodDA()).GetProgramSummaryOtherAdjsutmentDetails(adjNO, flag, histFlag, cmtcatGid);
        }
        //Phase-3
        public DataTable GetProgramSummaryPolicyInfo(int adjNO)
        {
            return (new ProgramPeriodDA()).GetProgramSummaryPolicyInfo(adjNO);
        }
        public DataTable GetRPAReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetRPAReport(adjNO, flag, histFlag);
        }

        public DataTable GetRPAAuditReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetRPAAuditReport(adjNO, flag, histFlag);
        }

        public DataTable GetRPAAuditReportSum(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetRPAAuditReportSum(adjNO, flag, histFlag);
        }

        public DataTable GetRPASummaryReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetRPASummaryReport(adjNO, flag, histFlag);
        }

        public DataTable GetRPALegendSummaryReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetRPALegendSummaryReport(adjNO, flag, histFlag);
        }

        public DataTable GetRPALegendCalcInfoReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetRPALegendCalcInfoReport(adjNO, flag, histFlag);
        }
        public DataTable GetRPALegendAuditedExposure(int adjNO)
        {
            return (new ProgramPeriodDA()).GetRPALegendAuditedExposure(adjNO);
        }

        public DataTable GetExcessLossReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetExcessLossReport(adjNO, flag, histFlag);
        }

        public DataTable GetILRFReport(int adjNO, int flag, bool histFlag, int cmtcatGid)
        {
            return (new ProgramPeriodDA()).GetILRFReport(adjNO, flag, histFlag, cmtcatGid);
        }

        public DataTable GetILRFReportExternal(int adjNO, int flag, bool histFlag, int cmtcatGid)
        {
            return (new ProgramPeriodDA()).GetILRFReportExternal(adjNO, flag, histFlag, cmtcatGid);
        }

        public DataTable GetILRFReportExternalSummaryTotal(int adjNO, int flag, bool histFlag, int cmtcatGid)
        {
            return (new ProgramPeriodDA()).GetILRFReportExternalSummaryTotal(adjNO, flag, histFlag, cmtcatGid);
        }

        public DataTable GetExcessLossExhibitExternalSummary(int adjNO, int flag, bool histFlag, int cmtcatGid)
        {
            return (new ProgramPeriodDA()).GetExcessLossExhibitExternal(adjNO, flag, histFlag, cmtcatGid);
        }
        public DataTable GetILRFFormula(int adjNO)
        {
            return (new ProgramPeriodDA()).GetILRFFormula(adjNO);
        }
        public DataTable CheckLDF_IBNR_Incl_Limit(int adjNO)
        {
            return (new ProgramPeriodDA()).CheckLDF_IBNR_Incl_Limit(adjNO);
        }


        public DataTable GetPaidLossBillingReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetPaidLossBillingReport(adjNO, flag, histFlag);
        }

        public DataTable GetSurchargesAssessmentReport(int adjNO, int flag, bool histFlag, bool revFlag)
        {
            return (new ProgramPeriodDA()).GetSurchargesAssessmentReport(adjNO, flag, histFlag, revFlag);
        }

        public DataTable GetLossFundAdjustmentReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetLossFundAdjustmentReport(adjNO, flag, histFlag);
        }

        public DataTable GetCesarCodingReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetCesarCodingReport(adjNO, flag, histFlag);
        }

        public DataTable GetLBAReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetLBAReport(adjNO, flag, histFlag);
        }

        public DataTable GetTexasTaxExternalReport(int adjNO, int flag, bool histFlag, int cmtcatGid)
        {
            return (new ProgramPeriodDA()).GetTexasTaxExternalReport(adjNO, flag, histFlag, cmtcatGid);
        }

        public DataTable GetTexasTaxInternalReport(int adjNO, int flag, bool histFlag, int cmtcatGid)
        {
            return (new ProgramPeriodDA()).GetTexasTaxInternalReport(adjNO, flag, histFlag, cmtcatGid);
        }

        public DataTable GetARiESPostingReport(int adjNO, int flag, bool histFlag)
        {
            return (new ProgramPeriodDA()).GetARiESPostingReport(adjNO, flag, histFlag);
        }
