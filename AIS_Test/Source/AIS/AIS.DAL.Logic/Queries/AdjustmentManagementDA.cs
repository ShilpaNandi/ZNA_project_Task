...
namespace ZurichNA.AIS.DAL.Logic
{
    public class AdjustmentManagementDA : DataAccessor<PREM_ADJ, AdjustmentManagementBE, AISDatabaseLINQDataContext>
    {
        ...
        /// <summary>
        /// This method is used for validating the program periods while doing the void.
        /// based on the output of this method we will allow the user to void or not.
        /// Added the following validation as part of the TFS bug 11812 permanent fix.
        /// </summary>
        /// <param name="intPREMADJID"></param>
        /// <returns></returns>
        public bool CheckDisbaledProgramPeriods(int  intPREMADJID)
        {
            if (this.Context == null)
                this.Initialize();
            ...
            IQueryable<PremiumAdjustmentPeriodBE> query =
                  (from cdd in this.Context.PREM_ADJ_PGMs
                   join pa in this.Context.PREM_ADJ_PERDs
                   on cdd.prem_adj_pgm_id equals pa.prem_adj_pgm_id
                   where pa.prem_adj_id == intPREMADJID
                   && cdd.actv_ind==false
