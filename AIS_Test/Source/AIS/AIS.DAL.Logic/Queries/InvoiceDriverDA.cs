...
namespace ZurichNA.AIS.DAL.Logic
{
    public class InvoiceDriverDA
    {
               ...
               //
                        ...
                        for (int i = 0; i < premAdjPerdBE.Count; i++)
                        {
                            ...
                            if ((PremAdj.valn_dt == pgmprdBE.nxt_valn_dt_non_prem_dt) && (PremAdj.valn_dt != pgmprdBE.nxt_valn_dt))
                            {
                                ...
                                //Added the following piece of code as part of the surcahrge project
                                //The Program period level inidcator value will be reset after the Finalize process.
                                //This will be performed to avoid the use of std subje prem in the subsequent adjustments
                                pgmprdBE.use_std_subj_prem_ind = false;
        ...
        //Phase-3
        public DataTable ProgramSummary_Cancel_ZDW_Info(int adjNO)
        {
            DataTable dtzdw = null;
            string connectionString = ConfigurationManager.ConnectionStrings["DBConnectionLINQ"].ToString();

            using (SqlConnection sqlcon = new SqlConnection(connectionString))
            using (SqlCommand sqlcmd = new SqlCommand("ProgramSummary_Cancel_ZDW_Info", sqlcon))
            {
                sqlcmd.CommandType = CommandType.StoredProcedure;
                sqlcmd.CommandTimeout = 0;
                sqlcmd.Parameters.Add("@Prem_Adj_id", SqlDbType.Int).Value = adjNO;
                sqlcon.Open();
        ...
        //Phase-3
        public DataTable GetProgramSummary_Info_ByZdwKey(string ZDWKeytxt)
        {
            DataTable dtzdw = null;
            string connectionString = ConfigurationManager.ConnectionStrings["DBConnectionLINQ"].ToString();

            using (SqlConnection sqlcon = new SqlConnection(connectionString))
            using (SqlCommand sqlcmd = new SqlCommand("GetProgramSummary_Info_ByZdwKey", sqlcon))
            {
                sqlcmd.CommandType = CommandType.StoredProcedure;
                sqlcmd.CommandTimeout = 0;
                sqlcmd.Parameters.Add("@ZDW_Key_Txt", SqlDbType.VarChar).Value = ZDWKeytxt;
                sqlcon.Open();
        ...
        //Phase-3
        public DataTable GetProgramSummary_ZDW_Info(int adjNO)
        {
            DataTable dtzdw = null;
            string connectionString = ConfigurationManager.ConnectionStrings["DBConnectionLINQ"].ToString();

            using (SqlConnection sqlcon = new SqlConnection(connectionString))
            using (SqlCommand sqlcmd = new SqlCommand("GetProgramSummary_ZDW_Info", sqlcon))
            {
                sqlcmd.CommandType = CommandType.StoredProcedure;
                sqlcmd.CommandTimeout = 0;
                sqlcmd.Parameters.Add("@Prem_Adj_id", SqlDbType.Int).Value = adjNO;
        ...
        //Phase-3
        public DataTable UpdateFinalZDWKey_ProgramSummary(int adjNO, string strZDWKey, int intUserID)
        {
            DataTable dtzdwFinal = null;
            string connectionString = ConfigurationManager.ConnectionStrings["DBConnectionLINQ"].ToString();

            using (SqlConnection sqlcon = new SqlConnection(connectionString))
            using (SqlCommand sqlcmd = new SqlCommand("ProgramSummary_Update_Final_ZDW_Info", sqlcon))
            {
                sqlcmd.CommandType = CommandType.StoredProcedure;
                sqlcmd.CommandTimeout = 0;
                sqlcmd.Parameters.Add("@ADJNO", SqlDbType.Int).Value = adjNO;
                sqlcmd.Parameters.Add("@FNL_ZDW_KEY", SqlDbType.VarChar).Value = strZDWKey;
                sqlcmd.Parameters.Add("@USERID", SqlDbType.Bit).Value = intUserID;
                sqlcon.Open();
                ...
                using (SqlDataReader reader = sqlcmd.ExecuteReader())
                {
                    if (reader.HasRows)
                    {
                        dtzdwFinal = new DataTable();
                        dtzdwFinal.Load(reader);
                    }
                }
            }
        ...
        //Phase-3
        public DataTable UpdateDraftZDWKey_ProgramSummary(int adjNO, string strZDWKey, int intUserID)
        {
            DataTable dtzdwDraft = null;
            string connectionString = ConfigurationManager.ConnectionStrings["DBConnectionLINQ"].ToString();

            using (SqlConnection sqlcon = new SqlConnection(connectionString))
            using (SqlCommand sqlcmd = new SqlCommand("ProgramSummary_Update_Draft_ZDW_Info", sqlcon))
            {
                sqlcmd.CommandType = CommandType.StoredProcedure;
                sqlcmd.CommandTimeout = 0;
                sqlcmd.Parameters.Add("@ADJNO", SqlDbType.Int).Value = adjNO;
                sqlcmd.Parameters.Add("@DRFT_ZDW_KEY", SqlDbType.VarChar).Value = strZDWKey;
                sqlcmd.Parameters.Add("@USERID", SqlDbType.Bit).Value = intUserID;
                sqlcon.Open();
                ...
                using (SqlDataReader reader = sqlcmd.ExecuteReader())
                {
                    if (reader.HasRows)
                    {
                        dtzdwDraft = new DataTable();
                        dtzdwDraft.Load(reader);
                    }
                }
            }
