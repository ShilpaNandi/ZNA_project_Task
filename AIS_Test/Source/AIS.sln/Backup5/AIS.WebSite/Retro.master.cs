...
public partial class Retromaster : System.Web.UI.MasterPage, ICallbackEventHandler
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       // lblwelcome.Text = " Welcome, [" + currentUser.Role + "] " + currentUser.UserID;
        lblwelcome.Text = " Welcome, [" + currentUser.Role + "] " + HttpContext.Current.User.Identity.Name;
    ...
    protected void TopNav_MenuItemClick(object sender, MenuEventArgs e)
    {
        ...
        SaveObjectToSessionUsingWindowName("MenuIdTier1", TopNav.SelectedValue);
        ...
        try
        {
            ...
            if (RetrieveObjectFromSessionUsingWindowName("MenuIdTier1") != null && RetrieveObjectFromSessionUsingWindowName("MenuIdTier1").ToString().Contains("/") == false)
            {
                //TopNav.FindItem(Session["MenuIdTier1"].ToString()).Selected = true;
                TopNav.FindItem(RetrieveObjectFromSessionUsingWindowName("MenuIdTier1").ToString()).Selected = true;
            }
        }
        catch
        {
    ...
    public string WindowName
    {
        get
        {
            if (((HiddenField)this.FindControl("form1").FindControl("tsttt").Controls[0].FindControl("hdnWindowName")) != null
                && ((HiddenField)this.FindControl("form1").FindControl("tsttt").Controls[0].FindControl("hdnWindowName")).Value != null
                && ((HiddenField)this.FindControl("form1").FindControl("tsttt").Controls[0].FindControl("hdnWindowName")).Value != "")
            {
                return ((HiddenField)this.FindControl("form1").FindControl("tsttt").Controls[0].FindControl("hdnWindowName")).Value;
            }
